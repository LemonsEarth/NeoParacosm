...
public class GapingThresher : ModItem
{
    ...
    public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
    {
        ...
        {
            LemonUtils.DustBurst(5, player.Center + velocity.SafeNormalize(Vector2.Zero) * 88, DustID.BlueCrystalShard, 3, 3, 2.2f, 2.3f);
            ...
            for (int i = -2; i <= 2; i++)
            {
                Projectile.NewProjectileDirect(
                    source,
                    position,
                    velocity.RotatedBy(Main.rand.NextFloat(-MathHelper.Pi / 12f, MathHelper.Pi / 12f)) * Main.rand.NextFloat(0.5f, 0.75f),
    ...
    public override void AddRecipes()
    {
        ...
        recipe.AddIngredient(ItemType<CrystalThresherHead>(), 1);
