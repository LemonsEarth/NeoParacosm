...
public class YellowLightning : ModProjectile
{
    ...
    public override void AI()
    {
        if (AITimer == 0)
        {
            ...
            SoundEngine.PlaySound(SoundID.Thunder with { PitchRange = (-0.2f, 0.2f), MaxInstances = 1, Volume = 0.35f }, Projectile.Center);
            ...
            while (Projectile.Center.Distance(targetPos) > 32)
            {
                ...
                Vector2 offset = normalVector * flip.ToDirectionInt() * Main.rand.Next(20, 36);
    ...
    public override bool PreDraw(ref Color lightColor)
    {
        ...
        VertexPositionColorTexture QuickVertexPCT(Vector2 pos)
        {
            ...
            return new VertexPositionColorTexture(new Vector3(pos, 0), Color.Yellow * Projectile.Opacity, Vector2.Zero);
        }
        ...
        for (int i = 0; i < 2; i++)
        {
            ...
            LemonUtils.DrawGlow(originalPos, Color.Yellow, circleOpacity, 2f);
            LemonUtils.DrawGlow(originalPos, Color.White, circleOpacity, 1f);
            LemonUtils.DrawGlow(positions.Last(), Color.Yellow, circleOpacity, 2f);
