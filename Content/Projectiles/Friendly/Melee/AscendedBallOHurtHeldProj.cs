...
public class AscendedBallOHurtHeldProj : ModProjectile
{
    int AITimer = 0;
    int releasedTimer = 0;
    bool released = false;
    ...
    public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
    {
        if (chargeCount >= 2 && released)
        {
            for (int i = 0; i < chargeCount; i++)
            {
                ...
                Projectile.NewProjectile(Projectile.GetSource_FromAI(), pos, Vector2.Zero, ProjectileType<Lightning>(), Projectile.damage / 3, 1f, Projectile.owner, ai1: target.Center.X, ai2: target.Center.Y);
            }
        }
    }
    ...
    public override void AI()
    {
        ...
        if (player.channel && !released)
        {
            ...
            if (chargeCount < 3 && AITimer % 60 == 0 && AITimer > 0)
            {
                chargeCount++;
