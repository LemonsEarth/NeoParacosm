...
public class SupremeRottedForkHeldProj : ModProjectile
{
    int AITimer = 0;
    int releasedTimer = 0;
    ...
    public override void AI()
    {
            ...
            if (AITimer % 30 == 0 && AITimer > 0)
            {
                if (addsLeft > 0)
                {
                    if (Main.myPlayer == Projectile.owner)
                    {
                        ...
                        LemonUtils.QuickProj(Projectile, Projectile.Center, Vector2.Zero, Type, Projectile.damage / ((addsLeft + 1) * 0.4f), ai0: -addsLeft);
                    }
                    addsLeft--;
                }
            }
        ...
        if (released)
        {
            ...
            if (savedMousePos == Vector2.Zero)
            {
                ...
                if (addsLeft >= 0)
                {
                    Projectile.velocity = dirToMouse * (15 * (8 - addsLeft));
                    Projectile.damage -= (int)(addsLeft * (0.2f * Projectile.damage));
                }
                else
                {
                    Projectile.velocity = dirToMouse * (15 * -addsLeft);
                }
