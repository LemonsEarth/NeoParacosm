...
namespace NeoParacosm.Content.Projectiles.Friendly.Ranged.HeldProjectiles;

public class SupremeMusketHeldProj : ModProjectile
{
    int AITimer = 0;
    ...
    public override void AI()
    {
        ...
        if (AITimer == 0)
        {
            ...
            {
                Fire(player);
                if (Main.rand.NextBool(2)) shotCount = Main.rand.Next(6, 8);
            }
            Projectile.netUpdate = true;
        }

        if (shotCount > 0 && AITimer > 0 && AITimer % 5 == 0)
        {
            if (Main.myPlayer == Projectile.owner)
            {
                Fire(player);
