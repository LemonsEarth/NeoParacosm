...
public class HolyBladeBuffItem : GlobalItem
{
    ...
    public override bool? UseItem(Item item, Player player)
    {
        ...
        if (player.HasBuff(BuffType<HolyBladeBuff>()) && item.shoot == ProjectileID.None && player.ItemAnimationJustStarted)
        {
            ...
            {
                float maxDamage = 0;
                if (player.GetElementalExpertiseBoost(SpellElement.Holy) >= 1.5f)
                {
                ...
                Vector2 velocity = player.DirectionTo(Main.MouseWorld) * 8 * player.GetElementalExpertiseBoost(SpellElement.Holy);
                float damage = player.GetDamage(DamageClass.Generic).ApplyTo(item.damage * (player.GetElementalDamageBoost(SpellElement.Holy) + 1));
